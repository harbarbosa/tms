...
export const validateCompanyForm = (values) => {
  ...
  if (!values.tipo_empresa) {
    errors.tipo_empresa = 'Selecione o tipo da empresa.'
  }
  ...
  ;['limite_usuarios', 'limite_transportadoras', 'limite_veiculos', 'limite_motoristas'].forEach((field) => {
    const value = Number(values[field] || 0)

    if (Number.isNaN(value) || value < 0) {
      errors[field] = 'Informe um limite valido.'
    }
  })
